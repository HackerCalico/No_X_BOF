...
该技术是我在 2024 年初首次提出的，当时发布了一个非常简单的样例来证明可行性。因为今年时间过于紧张，直到现在我才发布了 x64 版本，可以说是一个相当完备的版本。

我为此突破感到激动！

### 2. 规避优势

无需解密，无需 X 内存，直接加载 R 内存中的 ShellCode 密文。
...
ShellCode 功能的本质就是不断构建 Windows API 的参数 (寄存器的值、栈中的值、内存中的数据)，进而调用 Windows API。

只要实现一个虚拟空间 (虚拟寄存器、虚拟栈)，不断模拟每条汇编指令的功能，最终就可以在虚拟空间中构建出 Windows API 的参数。此时只要将虚拟值赋值到真实值，就可以成功调用 Windows API。

### 5. 项目文件说明

<u>ExecuteCmd.cpp</u>：CMD 命令执行 ShellCode。

<u>Converter.py</u>：自定义汇编指令转换器。

```bash
pip install capstone
```

<u>Loader.cpp</u>：ShellCode 加载器 (Clang 支持 x64 内联汇编)。
...
<u>如果操作数是 lea 的第二个操作数或内存空间</u>，例如 lq70+i20 ([rsp + 0x20]) 或 pq70+i20 (qword ptr [rsp + 0x20])。则先解析其子元素进行计算，计算结果保存到 number1。如果操作数是 lea 的第二个操作数，则 number1 的地址即为该操作数的地址。如果操作数是内存空间，则 number1 本身作为地址返回。

<u>如果操作数是寄存器</u>，例如 q70 (rsp)。70 是 vtRSP 相对 pVtRegs 基址的偏移，直接返回计算结果。
